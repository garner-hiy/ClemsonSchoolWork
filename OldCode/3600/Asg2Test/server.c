...
struct FileList getFileList() {
    ...
    // Add the header to formattedFileList
    size_t headerSize = snprintf(NULL, 0, "%-20s%10s\n", "Filename", "Size");
    fileData.formattedFileList = realloc(fileData.formattedFileList, headerSize + 1);
    if (fileData.formattedFileList == NULL) {
        perror("Memory re-allocation failed");
        exit(EXIT_FAILURE);
    }
    snprintf(fileData.formattedFileList, headerSize + 1, "%-20s%10s\n", "Filename", "Size");
    ...
    while ((directoryEntry = readdir(currentDirectory)) != NULL) {
        if (directoryEntry->d_type == DT_REG) {
            if (strstr(directoryEntry->d_name, ".txt") != NULL ||
                ...
                strstr(directoryEntry->d_name, ".cpp") != NULL) {
...
int main(int argc, char *argv[]){
    ...
    memset(data.downloadLog, 0, sizeof(data.downloadLog));
