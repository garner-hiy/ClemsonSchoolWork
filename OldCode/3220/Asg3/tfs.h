...
/* Masks for first two bits and last two bits of Status */
#define STATUS_MASK 0x03 // 00000011
// ( Status & R_MASK == R_MASK ) means read is set
#define R_MASK 0x40 // 01000000
// ( Status & W_MASK == W_MASK ) means write is set
...
// NOTE: status field will be used to hold Read/Write permissions
// READ permission will be held by 7th bit of status, WRITE held by 8th bit of status
// we will use masks to check these bitss
...
/* Read/Write/Status helper functions*/
...
//Used to see read/write permissions and also apply them
