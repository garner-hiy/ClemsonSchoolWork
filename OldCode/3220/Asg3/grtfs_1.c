...
void grtfs_list_directory(){
  ...
  for( fd = FIRST_VALID_FD; fd < N_DIRECTORY_ENTRIES; fd++ ){
    printf( "  fd = %2d: ", fd );
    if(grtfs_get_status(fd) == UNUSED) { //|| fd == 20) {
      printf( "unused\n" );
    ...
    if (  ((grtfs_get_status(fd) == CLOSED) ||
          (grtfs_get_status(fd) == OPEN))) { //&& fd != 20 ) {
      printf( "           FAT:" );
    ...
    // EDIT : Additional code added to display Read/Write permissions for 
    // each directory entry displayed
    if(grtfs_get_status(fd) != UNUSED) {
      if(file_is_readable(directory[fd].name)) 
        printf(" File is Readable"); 
      else 
        printf(" File is NOT Readable");
      if(file_is_writable(directory[fd].name))
        printf(" and is Writable\n");
      else
        printf(" and is NOT Writable\n");
    }
    // End additional code edit
  }
  printf( "-- end --\n" );
}
