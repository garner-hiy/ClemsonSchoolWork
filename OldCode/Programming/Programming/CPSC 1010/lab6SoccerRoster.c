...
int main (void){
    ...
    while (userOption != 'q'){//this loop will allow user to modify whatever till they quit
        ...
        int newRating;
        int arrayCheck1 = 0;//variable used to see if player is in the roster first
        
        
        switch(userOption){
            case 'p'://statement is for printing the roster
            ...
            case 'r'://this statement is for replacing a jersey and rating number
                ...
                for(int i = 0; i < numPlayers; ++i){//loop is to search the array 
                    if (jerseyNum[i] == jerseyToReplace){
                        arrayCheck1 = 1;//if player is in roster, it will update 
                    }
                }
                if (arrayCheck1 == 1){//if player was found then it will allow to modify
                    for(int i = 0; i < numPlayers; ++i){//this loop will allow user to modify if player was found
